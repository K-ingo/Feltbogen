...
describe('turforslag', () => {
  it('giver ingen forslag uden en tur', () => {
    expect(turforslag(null, [], [], [])).toEqual([]);
  });
  ...
  it('foreslår at kopiere en tidligere tur når der ikke er valgt grej endnu', () => {
    ...
    const forslag = turforslag(ny, [], [oekse], [gammel]);
    const kopi = forslag.find((f) => f.type === 'kopi');
  ...
  it('foreslår ikke at kopiere når turen allerede har grej', () => {
    ...
    expect(turforslag(ny, [], [oekse], [gammel]).some((f) => f.type === 'kopi')).toBe(false);
  });
  ...
  it('foreslår et grejsæt hvis tags rammer turens', () => {
    ...
    const gruppe = turforslag(tur, [saet], [], []).find((f) => f.type === 'gruppe');
  ...
  it('foreslår ikke et grejsæt der allerede er på turen', () => {
    ...
    expect(turforslag(tur, [saet], [], []).some((f) => f.type === 'gruppe')).toBe(false);
  });
  ...
  it('nævner kun vægten når der er noget at hente', () => {
    ...
    const vaegt = turforslag(tur, [], [tungt, let_], []).find((f) => f.type === 'vaegt');
  ...
  it('nævner ikke vægten når der ikke er et lettere alternativ', () => {
    ...
    expect(turforslag(tur, [], [tungt], []).some((f) => f.type === 'vaegt')).toBe(false);
  });
  ...
  it('viser højst tre forslag', () => {
    ...
    expect(turforslag(tur, [saet], [tungt, let_], []).length).toBeLessThanOrEqual(3);
  });
  ...
  it('regner ikke gear man har solgt eller overvejer med', () => {
    ...
    // Turen har intet ejet grej, så den skal behandles som tom.
    const gammel = lavTur({ navn: 'Sidste', status: 'afsluttet', loese_item_ids: [overvejet.uid] });
    expect(turforslag(tur, [], [overvejet], [gammel]).some((f) => f.type === 'vaegt')).toBe(false);
  });
});
